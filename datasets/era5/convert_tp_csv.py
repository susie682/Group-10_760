# convert_era5_tp_to_8hr_csv_multi.py
import xarray as xr
import numpy as np
import pandas as pd
import os
...
# ---------------------------
for year in years:
    tp_06_file = f'merged-nc-data/ERA5_{year}_06UTC_merged.nc'
    tp_18_file = f'merged-nc-data/ERA5_{year}_18UTC_merged.nc'
    ...
    for i, t in enumerate(ds_06['time'].values):
        hour_label = offset_06[i % 4]
        time_str = pd.Timestamp(t).strftime('%Y-%m-%d') + f'-{hour_label:02d}'
    ...
    for i, t in enumerate(ds_18['time'].values):
        hour_label = offset_18[i % 4]
        time_dt = pd.Timestamp(t)
        if hour_label < 6:
            time_dt -= pd.Timedelta(days=1)
        time_str = time_dt.strftime('%Y-%m-%d') + f'-{hour_label:02d}'
    ...
    # Output CSV to split-csv-data/year/
    year_dir = os.path.join(csv_root_dir, str(year))
    os.makedirs(year_dir, exist_ok=True)
    out_file = os.path.join(year_dir, f"totalPrecipitation_{year}.csv")
    df.to_csv(out_file)

...
grid = '1.0/1.0'
requests = [
    {'time': '06:00', 'steps': ['3','6','9','12']},
    {'time': '18:00', 'steps': ['3','6','9','12']}
]
...
for year in years:
    ...
    for month in range(1, 12):  # Jan~Nov
        ...
        for req in requests:
            fname = f'{raw_dir}/ERA5-{month_str}-tp_{req["time"].replace(":","")}.nc'
    ...
    for req in requests:
        fname = f'{raw_dir}/ERA5-{year}1201to1230-tp_{req["time"].replace(":","")}.nc'
        if os.path.exists(fname):
            print(f"File exists, skip: {fname}")
            continue
    ...
    for req in requests:
        ...
        if req['time'] == '18:00':
            steps = [s for s in steps if s not in ['9','12']]
        fname = f'{raw_dir}/ERA5-{year}1231-tp_{req["time"].replace(":","")}.nc'
        if os.path.exists(fname):
            print(f"File exists, skip: {fname}")
            continue
        c.retrieve(
    ...
    # Merge NC files
    # ---------------------------
    files_06 = sorted(glob.glob(f'{raw_dir}/ERA5-*-tp_0600.nc'))
    files_18 = sorted(glob.glob(f'{raw_dir}/ERA5-*-tp_1800.nc'))
    ...
    # Save merged data
    ds_06.to_netcdf(f'{merged_dir}/ERA5_{year}_06UTC_merged.nc')
    ds_18.to_netcdf(f'{merged_dir}/ERA5_{year}_18UTC_merged.nc')
